...
def palindromes(data):
    '''This function takes the refined data 
...
def main(input_path, output_csv):
    with open(output_csv, mode="w", newline="") as f:
        ...
        # decide if input is a file or a directory
        if os.path.isdir(input_path):
            dataset = [os.path.join(input_path, file) for file in os.listdir(input_path) if file.endswith('.out')]
        elif os.path.isfile(input_path) and input_path.endswith(".out"):
            dataset = [input_path]
        else:
            print("Error: input must be a .out file or a directory containing .out files")
            sys.exit(1)

        for file in tqdm(dataset):
            try:
                (seq_name, hairpins) = get_data(file, 10)
                ...
                for seq in rfam_pals:
                    ...
                    del rfam_graph, rfam_fp, rfam_counts
                    gc.collect()

                del rfam_pals, hairpins, seq_name, rfam_lengths, rfam_energies
                gc.collect()
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: python cosine_distribution.py <input_dir_or_file> <output_csv>")
        sys.exit(1)

    input_path = sys.argv[1]
    output_csv = sys.argv[2]
    main(input_path, output_csv)
